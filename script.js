Document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- NEW Auth Modal Utility Functions ---
    ...
    const BASE_URL = 'http://localhost:3000/api'; // This is your backend server URL
    ...
    const apiService = {
        ...
        fetchInitialGameData: async function() {
            try {
                // In a real app, you might send a token here to get user-specific data
                const response = await fetch(`${BASE_URL}/game/initial-data`);
                if (!response.ok) {
        ...
        updateGameStats: async function(stats) {
            try {
                // In a real app, you'd send a user token for authorization
                const response = await fetch(`${BASE_URL}/game/update-stats`, {
                    method: 'POST',
                    headers: {
        ...
        login: async function(username, password) {
            try {
                ...
                if (!response.ok) {
                    // If response is not ok, data will contain the error message from backend
                ...
                // Ensure a user-friendly message for network/parsing errors
                return { success: false, message: error.message || 'Login failed due to network error.' };
            }
        },

        /**
         * Attempts user registration on the backend. (Currently a placeholder)
         * You'll need to implement the /api/auth/register endpoint in server.js
         * @param {string} username
         * @param {string} password
        ...
        register: async function(username, password) {
            try {
                // This is a placeholder. You need to create a /api/auth/register endpoint on your backend.
                // For now, it will simply return a mock failure or success.
                console.log(`Attempting to register: ${username}, ${password}`);
                // Simulate an API call
                const response = await fetch(`${BASE_URL}/auth/register`, { // This endpoint does NOT exist in current server.js
                    method: 'POST',
    ...
    // Initial fetch and render of game data
    apiService.fetchInitialGameData()
        .then(data => {
            // Only update user data if the fetched data includes user info
            if (data.user && data.user.id) { // Check for user.id to ensure it's not the default guest
                Object.assign(gameState.user, data.user);
            }
            // Update score, level, coins regardless
            gameState.score = data.score;
            gameState.level = data.level;
            gameState.coins = data.coins;

            renderGameState(); // Render initial state
        })
        .catch(error => {
    ...
    // Play button animation and game loading sequence
    ...
    userAvatar.addEventListener('click', async function() {
        ...
        } else { // If user is a guest, show the login/register modal
            showAuthModal();
        }
    });
    ...
    loginBtn.addEventListener('click', async function() {
        ...
        if (loginResult.success) {
            ...
            hideAuthModal(); // Close the modal on successful login

            // After successful login, fetch the real game data for this user
            apiService.fetchInitialGameData()
                .then(data => {
    ...
    registerBtn.addEventListener('click', async function() {
        ...
        // --- Important: This currently calls a non-existent endpoint. ---
        // You MUST implement the /api/auth/register endpoint in your server.js
        // for actual registration to work.
    ...
    // Accessibility for dynamically updating stats
    ...
    // Simulate score and level updates (now interacting with gameState and REAL API service)
    setInterval(() => {
        ...
        // FIX: Added username to the stats object
        apiService.updateGameStats({
            username: gameState.user.username, // <--- THIS LINE IS THE FIX
            ...
            .then(response => {
                if (response.success) {
                    ...
                    // Optionally, you could re-sync gameState with backend's confirmed updatedStats here
                    // Object.assign(gameState, response.updatedStats); // If backend returns full updated state
                } else {
    ...
    // Simulate coin collection on general document click (now interacting with REAL API service)
    ...
    // Dynamically add common animation keyframes (keep as is)
    ...
    // Helper function to create coin collection visual effect (keep as is)
    ...
    // Helper function to create confetti effect (keep as is)
    ...
    // Helper function for level up visual effect (keep as is)
    ...
    // Helper function to create fireworks effect (keep as is)
